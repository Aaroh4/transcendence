...
import { User, MatchHistory } from "../services/api";
import { useToast } from "../components/toastBar/toastContext";
import { getUser, getMatchHistory } from "../services/userApi";
...
const FriendPage: React.FC = () => {
	...
	(async () => {
		...
		const matchResults = await getMatchHistory({ accToken }, id);
		...
		const opponentFetches = opponentIds.map(async (opponentId) => {
			try {
			const opponent = await getUser(String(opponentId));
			return { id: opponentId, name: opponent.name };
			} catch {
			return { id: opponentId, name: `User #${opponentId}` };
			}
      	});
	...
	<div className="flex gap-2 mt-10 mx-15">
			...
			<div className="w-2/5 border-2 border-black rounded-lg bg-[#2a2a2a] p-4">
				...
				<ul className="space-y-3 w-full max-h-[850px] overflow-y-auto pr-2">
					...
					const loggedInUserId = Number(id);
